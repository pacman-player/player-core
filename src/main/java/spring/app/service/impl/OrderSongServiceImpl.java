...
public class OrderSongServiceImpl implements OrderSongService {
    ...
    @Override
    public List<Long> getListSongIdByCompanyIdAndPeriod(Long id, Long period) {
        return orderSongDao.getListSongIdByCompanyIdAndPeriod(id, new Timestamp(System.currentTimeMillis() - period * 24 * 60 * 60 * 1000));
    }

    @Override
    public List<Long> getListSongIdByCompanyIdAndTimeRange(Long id, String period) {
        Calendar cal = Calendar.getInstance();
        cal.set(Calendar.HOUR_OF_DAY, 0);
        cal.set(Calendar.MINUTE, 0);
        cal.set(Calendar.SECOND, 0);
        ...
        Date yesterday = cal.getTime();
        Timestamp startOfTheDayYesterday = new Timestamp(yesterday.getTime());
        List<Long> result = orderSongDao.getListSongIdByCompanyIdAndTimeRange(id, startOfTheDayToday, new Timestamp(System.currentTimeMillis()));
        if (period.equals("yesterday")) {
            result = orderSongDao.getListSongIdByCompanyIdAndTimeRange(id, startOfTheDayYesterday, startOfTheDayToday);
        }
        return result;
    }
